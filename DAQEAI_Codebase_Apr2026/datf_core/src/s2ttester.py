...
class S2TTester:
    ...
    def execute_protocol(self, dict_protocol, df_testcases, output_path, combined_testcase_output_path, testcasetype,testcasesrunlist):
        ...
        if testcasetype == "count":
            ...
            df_protocol_summary = pd.DataFrame(columns=['Testcase Name', 'No of column has null in source','No of column has null in target',
                                    'No of null count match','No of null count mismatch', 'Test Result', 'Reason', 'Runtime'])
            ...
            try:
                ...
                if testcasetype == "count":
                    ...
                    df_protocol_summary.loc[index] = [test_case_name,str(dict_testresults['No of column has null in source']),str(dict_testresults['No of column has null in target']),
                                    str(dict_testresults['No of null count match']),str(dict_testresults['No of null count mismatch']),dict_compareoutput['test_result'],
                                                      dict_compareoutput['result_desc'], str(testcase_exectime)]
    ...
    def compare_data(self, compare_input, testcasetype):
        ...
        if (testcasetype == 'content'):
            ...
            comparison_obj = SparkSQLCompare(self.spark, sourcedf, targetdf, \
                                    join_columns=joincolumns,
                                    abs_tol=0,  # Optional, defaults to 0
                                    rel_tol=0,)  # Optional, defaults to 0
                                    #df1_name='org_src', df2_name='new_tgt')
            ...
            rows_mismatch      = comparison_obj.all_mismatch()
            rows_only_source   = comparison_obj.df1_unq_rows
            rows_only_target   = comparison_obj.df2_unq_rows

            # Row counts
            rowcount_common       = comparison_obj.intersect_rows.count()
            ...
            columns_match     = comparison_obj.intersect_columns()
            colcount_match    = len(columns_match)
            columns_compared  = list(comparison_obj.intersect_columns())
            ...
            dict_results = {'Test Result': test_result,
                            ...
                            'No. of cols in Source not Target': f"{colcount_source:,}",
                            'No. of cols in Target not Source': f"{colcount_target:,}",
                            ...
                            'No. of rows in Source not Target': f"{rowcount_only_source:,}",
                            'No. of rows in Target not Source': f"{rowcount_only_target:,}"
                            }
            ...
            print("Running duplicate check now...")
            ...
            dict_results = {
                'Test Result': test_result, 'No of column has null in source':f"{totalsrcnullcols:,}",'No of column has null in target':f"{totaltgtnullcols:,}", 'No of null count match':f"{columns_match_null_count:,}", 'No of null count mismatch': f"{columns_mis_null_count:,}"}
            ...
            if len(sourcenullcolumnlist) ==  0 and len(targetnullcolumnlist) == 0:
                    ...
                    nullcountmsg = 'Source and Target have zero nulls in columns.'
            
            elif len(sourcenullcolumnlist) == len(targetnullcolumnlist):    
                nullcountstatus = 'Passed'
                nullcountmsg = 'Source and Target have same amount of nulls in columns.'
...
'''
            ...
            coldict_results = {'Source Column Count': sourcecolcount, 'Target Column Count': targetcolcount, 'KPI Status':colcountstatus ,
                               'Reason': colcountmsg}
            nulldict_results = {'List of cols has null in source': sourcenullcolumnlist, 'List of cols has null in target': targetnullcolumnlist, 'KPI Status':nullcountstatus ,
                                'Reason': nullcountmsg}
            ...
            empdict_results = {'List of cols has empty in source': sourceempcolumnlist, 'List of cols has empty in target': targetempcolumnlist, 'KPI Status':empcountstatus ,
                               'Reason': empcountmsg }
