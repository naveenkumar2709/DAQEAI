...
def check_empty_values(df):
    # Replace nulls with empty strings for consistent trimming
    trimmed_df = df.select([F.trim(F.col(c)).alias(c) for c in df.columns])
    # Count empty strings for all columns in one go
    empty_counts = (
        trimmed_df
        .select([F.sum(F.when(F.col(c) == "", 1).otherwise(0)).alias(c) for c in trimmed_df.columns])
        .collect()[0]
        .asDict()
    )
    # Filter only columns having empty values
    columns_with_empty = [(k, v) for k, v in empty_counts.items() if v > 0]
...
def extract_database(qualified_name: str) -> str:
    """Extract database from db.schema.table or db.table"""
