...
def read_teradata(tc_datasource_config, spark):
    log_info("Reading from Teradata Table")
    ...
    if tc_datasource_config['testquerygenerationmode'] == 'Manual':
        ...
        df_oracledata.createOrReplaceTempView("teradataview")
        selectcolqry = "SELECT " + columnlist_str + " FROM teradataview"
        selectcolqry_ret = "SELECT " + columnlist_str + f" FROM {resourcename}"
        df_out = spark.sql(selectcolqry)

    df_out.printSchema()
    df_out.show()
    log_info("Returning the DataFrame from read_teradata Function")
