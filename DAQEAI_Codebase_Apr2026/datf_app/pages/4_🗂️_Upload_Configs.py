...
def upload_new_test_page():
    ...
    st.set_page_config(
        page_title="Upload Additional"
    )
    st.logo(logo_path)
    st.title("Upload Additional Configurations")
    ...
    jar_spark_file = st.file_uploader("JAR files (Spark/JDBC)",
                                          type='jar', accept_multiple_files=False)
    convention_jar = 'jar_'
    upload_status_jar = file_upload_all(jar_spark_file, 'jdbcdriver', convention_jar)
    if upload_status_jar is not None:
        if upload_status_jar == "issue1":
            st.error("Jar filename is already in use. Please rename and reupload.")
        elif upload_status_jar == "issue2":
            st.error(f"Filename must start with '{convention_jar}'. Please rename and reupload.")
        else:
            st.success(upload_status_jar)
